...
file_regex_collection = {'sam': '.*\.sam', 'fastq': '.*\.f\(ast\)?q', 'wig': '.*\.wig', 'bdg': '.*\.b\(e\)?dg\(raph\)?', 'public_raw_data': '[SE]RR.*\.f\(ast\)?q.gz'}
...
#find . -iregex "[SE]RR.*\.f\(ast\)?q.gz" > public_raw_data.files.txt