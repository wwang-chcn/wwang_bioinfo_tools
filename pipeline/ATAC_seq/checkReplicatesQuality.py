    ...
    with open(running_file) as fhd:
        for line in fhd:
            if not line.startswith('bash'):
                continue
            _, _, name, processer, genome_version, reads1_files, reads2_files, *_ = line.strip().split()
...
def check_target_regions_file(genome_version, genome_bin_file=None, tss_3000_file=None):
    """
        ...
        with open(genome_size_file) as input_fhd, \
            ...
            for line in input_fhd:
                ...
                for i in range(int(length) // 5000 + 1):
                    index += 1
                    output_fhd.write(f'{chrom}\t{i*5000}\t{i*5000+5000}\tbin_{index}\n')
    if not os.path.isfile(tss_3000_file):
        ...
        def promoter_gen(fhd, promoter_range=3000):
            for index, line in enumerate(fhd):
                ...
                new_line = [
                    chrom, f'{max(TSS-promoter_range, 0):d}', f'{TSS+promoter_range:d}', f'{name}_{index+1}', '0',
                    strand
                ]
                yield '\t'.join(new_line) + '\n'
...
def get_bigwig_mean(bigwig_file):
    ...
    """
    import subprocess
    fold = subprocess.check_output(f'bigWigInfo {bigwig_file} | grep mean', shell=True).decode()
...
def get_bigwig_files(samples):
    ...
    for sample in samples:
        bigwig_file = f'../2_signal/OCR/{sample}_OCR.bw'
...
def avg_bed_signal_capture(name, target_regions_file, bigwig_files, labels, parallel_num=8):
    """
...
def process_signal(name, target_regions_file, bigwig_files, labels):
    ...
    for label, bigwig_file in zip(labels, bigwig_files):
        ...
        capture_signal[label] = pd.read_csv(f'{label}_signal.tsv',
                                            sep='\t',
                                            header=None,
                                            index_col=0,
    ...
    if corr.shape[0] == 1:
        print('Only one replicate, skip correlation analysis')
        print(f'Replicates with highest correlation are: {labels[0]}')
    elif corr.shape[0] == 2:
        # less than 3 replicates
        print('Less than 3 replicates, skip correlation analysis')
        replicates = labels
        corr_value = corr.values[0][1]
        ...
        corr_3 = sorted([[key, value] for key, value in corr_3.items()], key=lambda x: x[1], reverse=True)
...
def main():
    ...
    os.rename(src='ATAC_summary_replicates_check_fragments_length.csv',
              dst='replicates_quality_check/ATAC_summary_replicates_check_fragments_length.csv')
    os.rename(src='ATAC_summary_replicates_check_mapping_info.csv',
              dst='replicates_quality_check/ATAC_summary_replicates_check_mapping_info.csv')
    #
    os.chdir('replicates_quality_check')
    for condition, samples in replicates.items():
        bigwig_files = get_bigwig_files(samples)
        avg_bed_signal_capture(condition, genome_bin_file, bigwig_files, samples)
        process_signal(condition, genome_bin_file, bigwig_files, samples)
        avg_bed_signal_capture(condition, tss_3000_file, bigwig_files, samples)
