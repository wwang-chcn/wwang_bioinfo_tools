...
def main():
    ...
    with open(f'ATAC_summary_{outputPrefix}_mapping_info.csv', 'w') as mappingFhd, \
        ...
        mapping_csv.writerow([
            ...
            'Mapping Efficiency', 'chrM Fragments', 'chrM Percentage',
        ...
        for name, genomeVersion, reads1_files, reads2_files in load_sample(
            ...
            output_line.extend(
                [
                    total, mapped, 1.0 * mapped / total, chrM,
                    1.0 * chrM / mapped
                ]
            )  # Raw reads pair \t mapped reads pair \t mapping efficiency \t chrM \t chrM percentage

            # unique nuclear fragments & duplicate level
            raw_fragment = int(
                subprocess.check_output(
                    f'''bigBedInfo 2_signal/{sample}_raw_fragments.bb | grep itemCount | cut -d " " -f 2 | sed -s 's/,//g' ''',
                    shell=True).decode().strip())
            fragment = int(
                subprocess.check_output(
                    f'''bigBedInfo 2_signal/{sample}_fragments.bb | grep itemCount | cut -d " " -f 2 | sed -s 's/,//g' ''',
                    shell=True).decode().strip())
            output_line.extend([
