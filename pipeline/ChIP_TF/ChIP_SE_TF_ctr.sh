...
function mapping_filtering {
    ...
    if [[ ! -e 2_signal/${controlName}_reads.bed ]]; then
        ...
        trim_galore --fastqc --fastqc_args "--outdir 0_raw_data/FastQC_OUT --nogroup -t ${processer} -q" -j 4 --trim-n -o 0_raw_data/ --no_report_file --suppress_warn ${trim_galore_input[@]} 2>&1 | tee 0_raw_data/Trim_galore_${controlName}.log && \
        ...
        rm ${filteredReads[@]} && \
        bamToBed -i 1_mapping/${controlName}.bam | awk '$1 !~ /_/{if($3>$2) print}' > 2_signal/${controlName}_reads.bed
    fi
}
