...
function mapping_filtering {
    ...
    if [[ ! -e 2_signal/${controlName}_reads.bed ]]; then
        ...
        bamToBed -i 1_mapping/${controlName}.bam | awk '$1 !~ /_/{if($3>$2) print} $1 ~ /NC/{if($3>$2) print}' | sort -S 1% -k1,1 -k2,2g | uniq > 2_signal/${controlName}_reads.bed
    fi
}
...
function peak_calling {
    ...
    macs3 callpeak -f BED -t 2_signal/${name}_reads.bed -c 2_signal/${controlName}_reads.bed --outdir 3_peak -n ${name} -g ${chromsize} -q 0.05 2>&1 >>/dev/null | tee 3_peak/${name}_MACS.out
}
