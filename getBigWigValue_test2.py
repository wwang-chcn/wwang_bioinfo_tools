...
# Notes
# try using capture all datpoints in one bed file
# a little slower than seperated capture in capture stage
# most time used in post-processing (four times slower in this step)
...
def prepare_capture(options):
    ...
    for i in range(options.datapoints):
        tmp_fhd[i].close()
    # creat all regions in one bed file
    CMD = f'cd {options.name}\n'
    CMD += 'cat'
    for i in range(options.datapoints):
        CMD += f' tmp_{i}.bed'
    CMD += ''' | awk '{print $1"\t"$2"\t"$3"\t"NR"\t"$5"\t"$6}' > tmp.bed\n'''
    # load bigwig files
    for i in range(len(options.bigWigFiles)):
...
def capture_signal(options):
    ...
    for bigwig_file_index, bigwig_file in enumerate(options.bigWigFiles):
        bed_file = 'tmp.bed'
        output_file = f'tmp_{bigwig_file_index}.tsv'
    ...
    for bigwig_file_index, bigwig_file in enumerate(options.bigWigFiles):
        signal = pd.read_csv(f'{options.name}/tmp_{bigwig_file_index}.tsv', header=None, sep='\t', names=['name','size','covered','sum','mean0','mean'])
        signal_matrix = pd.DataFrame(signal['mean'].to_numpy().reshape((-1,options.datapoints)))
